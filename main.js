...
function renderZone(zoneKey) {
    ...
    // remove previous injected employees & badges
    domEl.querySelectorAll(".zone-employee").forEach(n => n.remove());
    domEl.querySelectorAll(".capacity-badge").forEach(n => n.remove());

    // occupants for this zone
    const occupants = employes.filter(e => e.assigned && e.zone === zoneKey);

    // mandatory highlight
    if (cfg.mandatory && occupants.length === 0) domEl.classList.add("bg-red-50");
    else domEl.classList.remove("bg-red-50");

    // capacity badge
    const capBadge = document.createElement("div");
    capBadge.className = "capacity-badge text-xs mt-1";
    capBadge.innerText = `${occupants.length}/${cfg.capacity}`;
    domEl.appendChild(capBadge);

    // append occupants nodes
...
function showdetails(empId) {
  ...
  `;
    ...
    if (!emp.exprecinces || emp.exprecinces.length === 0) {
        expList.innerHTML = "<p class='text-sm text-gray-500'>Aucune expérience renseignée.</p>";
    } else {
